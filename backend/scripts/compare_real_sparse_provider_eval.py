"""真实知识库下比较不同稀疏检索 provider 的问答评测表现。

用于判断 sqlite_fts、bm25、scan 或关闭稀疏召回时是否影响最终答案。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build real PDF knowledge bases for multiple sparse providers, run eval_rag on each, then compare."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    parser.add_argument(
        "--label",
        default="real-sparse-provider-ab",
    ...
    parser.add_argument(
        "--name-prefix",
        default="Real Sparse Provider Eval",
...
async def _build_eval_report(
    *,
    ...
    knowledge_base = knowledge_base_service.create_knowledge_base(
        ...
        description=f"real sparse provider eval for {provider_label}",
